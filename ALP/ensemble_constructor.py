...
class Ensemble():
    ...
    def init(self):
        ...
        if self.learner_fqn == 'sklearn.neighbors._classification.KNeighborsClassifier':
            num_neighbors = np.random.choice(np.linspace(1, self.max_neighbors, self.max_neighbors, dtype=int),
                                             self.num_estimators)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        None
        """
        if self.learner_fqn in ["sklearn.ensemble._forest.RandomForestClassifier",
