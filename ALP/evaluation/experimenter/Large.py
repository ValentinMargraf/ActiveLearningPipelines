...
exp_learner_sampler_file = "config/sparse_large.yml"
...
class ExperimentRunner:
    ...
    def run_experiment(self, parameters: dict, result_processor: ResultProcessor, custom_config: dict):
        ...
        setting = ActiveLearningSetting(setting_id=3, setting_name='large', setting_test_size=0.33,
                              setting_train_type='absolute', setting_labeled_train_size=300,
                              number_of_iterations=20, number_of_samples=10, factor = 20)
        ...
        # i.e. if setting_name == 'small' then factor = 2
        # num_iter
        # i.e. get_number_of_samples = 2 * #classes // num_iter
        # to discuss with Sandra
...
def run_parallel(variable, default_setup=False, run_setup=False, reset_experiments=False):
    ...
    if run_setup:
        ...
        filter_out = [1567, 1169, 41147, 1493, 11, 12, 14, 16, 18, 22, 25, 51, 54, 188, 307, 458, 469, 1468, 1501, 40966, 40979]

        setting_combinations += [{'setting_name': 'large', 'openml_id': oid, 'test_split_seed': seed, 'train_split_seed': seed} for oid in all_ids if oid not in filter_out for seed in np.arange(10)]


        if reset_experiments:
            experimenter.reset_experiments('failed', 'error','created', 'running')
...
def main():
    ...
    run_parallel(variable, run_setup = True, reset_experiments = False)
